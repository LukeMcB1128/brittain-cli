...
function createContextInspector(rt) {
  async function inspect({ model, cwd } = {}) {
    try {
      const view = rt.session.view;
      const useModel = model || rt.providers.resolve().model;
      const useCwd = cwd || view.cwd || rt.config.cwd;
      const prompt = rt.prompts.systemPrompt(useCwd, useModel);
      ...
      // single component, so they belong in the total.
      const toolDefs = rt.prompts.activeToolDefs();
...
function formatContext(result, { style = { dim: (t) => t, bold: (t) => t } } = {}) {
  ...
  const lines = [
    style.bold(`Context: ${n(result.totalTokens)} of ${n(result.contextLength)} tokens (${result.percentUsed}%)`) + style.dim(` · ${result.model}`),
